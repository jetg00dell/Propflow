...
export default async function LeaseDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="min-h-screen bg-gray-950 p-6">
      <div className="mb-6">
        <Link href="/leases" className="text-gray-400 hover:text-white text-sm transition-colors flex items-center gap-1">
          ← Back to Leases
        </Link>
      </div>
      ...
      <div className="mb-6 flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold text-white">{propertyLabel}</h1>
          {property && (
            <p className="text-gray-400 mt-0.5">{property.address}, {property.city}, {property.state}</p>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2">
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
            <h2 className="text-white font-semibold text-base mb-4">Lease Terms</h2>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              ...
              {(lease.pet_deposit as number | null) != null && (lease.pet_deposit as number) > 0 && (
                <div>
                  <p className="text-gray-500 text-xs uppercase tracking-wider mb-1">Pet Deposit</p>
                  <p className="text-white font-semibold">${(lease.pet_deposit as number).toLocaleString()}</p>
                </div>
              )}
              {(lease.late_fee_flat as number | null) != null && (lease.late_fee_flat as number) > 0 && (
                <div>
                  <p className="text-gray-500 text-xs uppercase tracking-wider mb-1">Late Fee</p>
                  <p className="text-white font-semibold">${(lease.late_fee_flat as number).toLocaleString()}</p>
                </div>
              )}
              <div>
                <p className="text-gray-500 text-xs uppercase tracking-wider mb-1">Start Date</p>
                <p className="text-gray-300 text-sm">{formatDate(lease.start_date as string | null)}</p>
              </div>
              <div>
              ...
              {lease.renewal_notice_days != null && (
                <div>
                  <p className="text-gray-500 text-xs uppercase tracking-wider mb-1">Renewal Notice</p>
                  <p className="text-gray-300 text-sm">{lease.renewal_notice_days as number} days</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 h-fit">
          <h2 className="text-white font-semibold text-base mb-4">Tenants</h2>
          {leaseTenantsList.length === 0 ? (
            <p className="text-gray-500 text-sm italic">No tenants on record.</p>
          ) : (
              ...
              {leaseTenantsList.map((t: any) => (
                <div key={t.id}>
                  <div className="flex items-center gap-2">
                    ...
                    {t.is_primary && (
                      <span className="text-gray-500 text-xs border border-gray-700 rounded px-1.5 py-0.5 leading-none">
                        Primary
                      </span>
                    )}
                  </div>
