...
export default async function TenantDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="min-h-screen bg-[#F5F6FA] p-6">
      <div className="mb-6">
        <Link href="/tenants" className="text-gray-500 hover:text-[#1C7BC0] text-sm transition-colors flex items-center gap-1">
          ← Back to Tenants
        </Link>
      </div>

      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-[#1A2B4A]">{tenant.first_name} {tenant.last_name}</h1>
        {tenant.email && <p className="text-gray-500 mt-0.5">{tenant.email}</p>}
        {tenant.phone && <p className="text-gray-500 text-sm">{tenant.phone as string}</p>}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-6">
            <h2 className="text-[#1A2B4A] font-semibold text-base mb-4">Current Lease</h2>
            {lease ? (
              <>
                {property && (
                  <div className="mb-5">
                    <Link
                      ...
                      className="text-[#1C7BC0] hover:text-[#1669A8] font-medium transition-colors"
                    >
                      {property.name}
                    </Link>
                    {unit?.unit_number && (
                      <span className="text-gray-500 text-sm ml-2">· Unit {unit.unit_number}</span>
                    )}
                  </div>
                )}
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-gray-700 text-sm">{formatDate(lease.start_date)}</p>
                  </div>
                  <div>
                ...
                {coTenants.length > 0 && (
                  <div className="mt-5 pt-5 border-t border-gray-100">
                    <p className="text-gray-500 text-xs uppercase tracking-wide mb-2">Co-Tenants</p>
                      ...
                      {coTenants.map((ct: any) => {
                        ...
                        return (
                          <div key={t.id} className="flex items-center gap-3">
                            <Link
                              href={`/tenants/${t.id}`}
                              className="text-[#1C7BC0] hover:text-[#1669A8] text-sm transition-colors"
                            >
              ...
              <p className="text-gray-400 text-sm italic">No active lease on file.</p>
            )}
          </div>

          {/* Personal info card */}
          <div className="bg-white border border-gray-200 rounded-xl p-6">
            <h2 className="text-[#1A2B4A] font-semibold text-base mb-4">Personal Information</h2>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              <div>
                <p className="text-gray-500 text-xs uppercase tracking-wide mb-1">Employer</p>
                <p className="text-gray-700 text-sm">{(tenant.employer as string | null) ?? '—'}</p>
              </div>
              <div>
                <p className="text-gray-500 text-xs uppercase tracking-wide mb-1">Monthly Income</p>
                <p className="text-gray-700 text-sm">
              ...
              <div>
                <p className="text-gray-500 text-xs uppercase tracking-wide mb-1">Credit Score</p>
                <p className="text-gray-700 text-sm">{(tenant.credit_score as number | null) ?? '—'}</p>
              </div>
              <div>
                <p className="text-gray-500 text-xs uppercase tracking-wide mb-1">Background Check</p>
                <span className={`text-xs px-2 py-0.5 rounded-full ${
              ...
              {tenant.background_check_date && (
                <div>
                  <p className="text-gray-500 text-xs uppercase tracking-wide mb-1">Check Date</p>
                  <p className="text-gray-700 text-sm">{formatDate(tenant.background_check_date as string)}</p>
                </div>
              )}
            </div>
          </div>
        ...
        {tenant.emergency_contact_name && (
          <div>
            <div className="bg-white border border-gray-200 rounded-xl p-6">
              <h2 className="text-[#1A2B4A] font-semibold text-base mb-4">Emergency Contact</h2>
              <div className="space-y-3">
                <div>
                  <p className="text-gray-500 text-xs uppercase tracking-wide mb-1">Name</p>
                  <p className="text-gray-700 text-sm">{tenant.emergency_contact_name as string}</p>
                </div>
                {tenant.emergency_contact_phone && (
                  <div>
                    <p className="text-gray-500 text-xs uppercase tracking-wide mb-1">Phone</p>
                    <p className="text-gray-700 text-sm">{tenant.emergency_contact_phone as string}</p>
                  </div>
                )}
              </div>
            </div>
