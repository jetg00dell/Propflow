...
function sortTenants(rows: TenantRow[], col: SortCol, dir: SortDir): TenantRow[] {
  return [...rows].sort((a, b) => {
    ...
    if (col === 'tenant') {
      ...
      // lease_end: nulls always to bottom
    ...
    <div className="min-h-screen bg-gray-950 p-6">
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">{stats.totalTenants} tenants</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
          ...
          <p className="text-white text-xl font-bold">{stats.totalTenants}</p>
        </div>
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
          <p className="text-gray-400 text-xs uppercase tracking-wider mb-1">Active Leases</p>
          <p className="text-white text-xl font-bold">{stats.activeLeases}</p>
        </div>
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
          <p className="text-gray-400 text-xs uppercase tracking-wider mb-1">Avg Credit Score</p>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
          ...
          <p className="text-white text-xl font-bold">${stats.monthlyRevenue.toLocaleString()}</p>
        </div>
      </div>
              ...
              {sorted.map(t => (
                <tr key={t.id} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                  ...
                  <td className="px-5 py-3">
                    {t.property_name ? (
                      <div>
                        <p className="text-gray-300 text-sm">{t.property_name}</p>
                        {t.unit_number && (
                  ...
                  <td className="px-5 py-3 text-right">
                    <span className="text-gray-300 text-sm">{t.credit_score ?? '—'}</span>
                  </td>
                </tr>
              ))}
            </tbody>
