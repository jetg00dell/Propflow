...
import {
  ...
  Building2, Menu, X, LogOut, Search,
...
export default function Dashboard({ units, recentPayments, maintenanceItems, expiringLeases }: {
  ...
  const stats = [
    ...
    },
    {
      ...
      sub: `${occupiedUnits} / ${totalUnits} units occupied`, trend: "neutral", trendVal: "",
    },
    {
    ...
    <div className="flex h-screen bg-[#F5F6FA] overflow-hidden">
      ...
      <aside className={`flex-shrink-0 flex flex-col bg-white border-r border-gray-200 transition-all duration-300 ${sidebarOpen ? "w-60" : "w-16"}`}>
        ...
        <div className="p-3 border-t border-gray-200 space-y-0.5">
          ...
          <button onClick={handleSignOut} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-gray-600 hover:bg-[#F0F7FF] hover:text-[#1C7BC0] transition-colors">
                    ...
                    {expiringLeases.map((l, i) => {
                      ...
                      return (
                        <div key={i} className="flex items-start gap-3 cursor-pointer hover:opacity-80 transition-opacity">
                          <div className={`w-1 h-1 rounded-full mt-2 flex-shrink-0 ${days < 45 ? "bg-red-600" : "bg-amber-500"}`} />
                          <div className="flex-1 min-w-0">
